...
from ..shared.compat import FontBold, DialogAccepted
from ..shared.constants import COMPACT_BUTTON_MAX_HEIGHT, COMPACT_BUTTON_SMALL_MAX_HEIGHT
...
class SmartFilterWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        font = self.enable_filter_cb.font()
        font.setBold(True)
        self.enable_filter_cb.setFont(font)
        ...
        apply_compact_button(self.test_filter_btn, max_width=50, max_height=COMPACT_BUTTON_SMALL_MAX_HEIGHT)
        
        self.builder_btn = QPushButton("Builder...")
        self.builder_btn.setEnabled(False)
        apply_compact_button(self.builder_btn, max_width=60, max_height=COMPACT_BUTTON_SMALL_MAX_HEIGHT)
        ...
        for name, template in templates:
            ...
            apply_compact_button(btn, max_width=65, max_height=COMPACT_BUTTON_MAX_HEIGHT)
    ...
    def on_filter_expression_changed(self):
        ...
        self.emit_filter_changed()  # Emit filter_applied signal to update configuration
    ...
    def test_filter(self):
        ...
        try:
            # Test expression
            is_valid, error_msg, expression = validate_expression_syntax(filter_expr)
            if not is_valid:
                QMessageBox.warning(self, "Filter Error", f"Syntax error: {error_msg}")
                return
...
Filter is valid and ready to use!"""
    ...
    def clear_filter(self):
        """Clear the filter expression and disable filtering"""
        self.enable_filter_cb.setChecked(False)
        self.filter_expression.clear()
    
    def set_filter_expression(self, expression):
        """Set filter expression and enable filtering if expression is provided"""
        if expression and expression.strip():
            self.enable_filter_cb.setChecked(True)
            self.filter_expression.setText(expression.strip())
        else:
            self.clear_filter()
