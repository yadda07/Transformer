...
class EnhancedTransformerDialog(QMainWindow):
    ...
    def _apply_table_config(self, table_name: str, config: dict, filename: str):
        """Apply a specific table configuration"""
        if not config:
            return
    ...
    def _show_config_selector(self, table_names: list, filename: str):
        ...
        try:
            if not SMOOTH_CONFIG_SELECTOR_AVAILABLE or SmoothConfigSelector is None:
                ...
                self._apply_table_config(first_table, config, filename)
    ...
    def _on_config_selected(self, table_name: str, filename: str):
        ...
        self._apply_table_config(table_name, config, filename)
    ...
    def refresh_shapefile_list(self):
        ...
        for filename, data in self.loaded_shapefiles.items():
            ...
            print(f"DEBUG: {filename} - is_qgis_layer: {is_qgis_layer}")
            ...
            if is_qgis_layer:
                ...
                print(f"DEBUG: Couleur bleue appliquée à {filename}")
                
                # Icône selon géométrie
    ...
    def remove_selected_shapefile(self):
        """Remove the selected shapefile(s) - supports multiple selection"""
        selected_items = self.shp_tree.selectedItems()
        if not selected_items:
            self.log_message("Please select one or more shapefiles to remove", "Warning")
            return
        
        # Confirmation message for multiple selection
        if len(selected_items) == 1:
            filename = selected_items[0].data(0, Qt.UserRole)
            confirm_msg = f"Remove '{filename}' from the source files list?"
        else:
            confirm_msg = f"Remove {len(selected_items)} selected files from the source files list?"
        ...
        if removed_count > 0:
            self.log_message(f"Removed {removed_count} file(s) from source list", "Success")
            ...
            for item in selected_items:
                ...
                if filename in self.loaded_shapefiles:
                    try:
                        ...
                        self.config_manager.add_table_config(
                            file_table_name,
                            source_file,
                            calculated_fields,
                            filter_config,
    ...
    def on_shapefile_selection_changed(self):
        ...
        if current:
            ...
            # This allows seeing individual shape fields even after Copy All
    ...
    def _show_config_selector(self, table_names, filename):
        """Show smooth config selector for multiple configurations"""
        if SmoothConfigSelector is None:
            # Fallback to first configuration if selector unavailable
            self.log_message("Multiple configurations found - loading first one", "Info")
            first_table = table_names[0]
            config = self.config_manager.get_table_config(first_table)
            if config:
                self._apply_table_config(first_table, config)
    ...
    def _apply_table_config(self, table_name, config):
        """Apply a configuration to the interface"""
        try:
            self.table_name_edit.setText(table_name)
            
            # Load calculated fields and geometry expression
            calculated_fields = config.get('calculated_fields', {})
            geometry_expression = config.get('geometry_expression', '$geometry')
            self.log_message(f"DEBUG: Loading {len(calculated_fields)} calculated fields and geometry_expression: {geometry_expression}", "Info")
            self.smart_fields.set_calculated_fields(calculated_fields, geometry_expression)
            ...
            self.log_message(f"Loaded saved configuration for '{table_name}'", "Info")
            self.update_configuration_preview()
