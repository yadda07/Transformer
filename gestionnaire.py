...
class SimpleConfigManager:
    ...
    def add_table_config(self, table_name: str, source_file: str, calculated_fields: Dict[str, str], 
        ...
        Returns:
            bool: True if configuration was added/updated, False if rejected due to existing config
        """
        if "tables" not in self.config_data:
            self.config_data["tables"] = {}
        
        # Check if configuration already exists for this table
        if table_name in self.config_data["tables"] and not force_replace:
            ...
            QgsMessageLog.logMessage(
                f"Configuration already exists for table '{table_name}' (source: {existing_source}). "
                f"Use force_replace=True to update or choose a different table name.", 
        ...
        action = "Updated" if table_name in self.config_data["tables"] else "Added"
        QgsMessageLog.logMessage(
            f"{action} config for {table_name}: {len(calculated_fields)} fields{filter_info}", 
            "Transformer", Qgis.Info
        )
    ...
    def replace_table_config(self, table_name: str, source_file: str, calculated_fields: Dict[str, str], 
        ...
        Returns:
            bool: True if configuration was replaced successfully
        """
    ...
    def get_all_tables(self) -> Dict[str, Dict]:
        ...
        # Ensure all tables have filter config
